...
namespace sce {
  ...
  template <typename dT>
  std::ostream &operator<<(std::ostream &out, __struct<dT> s);
  template <typename dT>
  std::istream &operator>>(std::istream &in, __struct<dT> s);

  template <typename T>
  class comparator {
   protected:
    virtual int64_t operator()(T a, T b);

   public:
    comparator();
    comparator(int64_t (*func)(T a, T b));
